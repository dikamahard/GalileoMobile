...
public class TermsConditionTest {
    ...
    public void privacyPolicyTest() throws InterruptedException {
        TermsConditionPage termsConditionPage = new TermsConditionPage(driver);
        ...
        driver.findElement(AppiumBy.androidUIAutomator("new UiScrollable(new UiSelector().scrollable(true)).scrollIntoView(new UiSelector().description(\"Terms & Condition\"))")).click();

        new WebDriverWait(driver, Duration.ofSeconds(10)).until(ExpectedConditions.visibilityOfElementLocated(termsConditionPage.tvTitle));

        assertTrue(termsConditionPage.getTvTitle().isDisplayed());
    }
}
